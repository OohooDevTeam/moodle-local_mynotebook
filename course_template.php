...
if ($coursename==''){
    echo "<center><input type='button' value='Go back and enter a course.' onclick='history.back(-1)' /></center>";
    exit();
}
...
$notes = $DB->get_records('notes', array('courseid'=>$course->id, 'userid'=>$USER->id, 'deleted'=>0));
...
$TBS->MergeBlock('c', $note_data);
    ...
    for ($j=0; $j<sizeof($arrayid); $j++){
        $record_amount[$j] = $DB->count_records('notes', array('courseid'=>$arrayid[$j],'userid'=>$USER->id, 'deleted'=>0));
    }
...
//    $TBS->Show(OPENTBS_DOWNLOAD + OPENTBS_DEBUG_XML, $file_name);
}
